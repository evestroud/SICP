...
(define (install-polynomial-package)
  ...
  (define (install-dense-term-list-package)
    ...
    (define (adjoin-term-dense term term-list)
      (if (=zero? (coeff term))
          term-list
          (let ((order-difference (- (order term) (length term-list))))
            (append (cons (coeff term) (make-list order-difference (make-integer 0)))
                    term-list))))
    (define (empty-termlist?-dense term-list)
      (null? term-list))
    (define (first-term-dense term-list)
      (let ((order (1- (length term-list)))
            (term (car term-list)))
        (make-term order term)))
    (define (rest-terms-dense term-list)
      (or
       (find-tail (lambda (term) (not (=zero? term))) (cdr term-list))
       '()))
    (put 'adjoin-term '(dense-term-list)
         (lambda (term term-list) (tag (adjoin-term-dense term term-list))))
    (put 'empty-termlist? '(dense-term-list) empty-termlist?-dense)
    (put 'first-term '(dense-term-list) first-term-dense)
